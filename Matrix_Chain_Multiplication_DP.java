import java.util.*;
public class Matrix_Chain_Multiplication_DP {
    public static int MCMrecurssion(int arr[] , int i , int j){
        if(i==j){
            return 0;
        }
        int ans = Integer.MAX_VALUE;
        for(int k =i ;k<=j-1;k++){
            int cost1 = MCMrecurssion(arr,i,k);
            int cost2=MCMrecurssion(arr,k+1,j);
    ...
    public static void main(String[] args) {
        int arr[] = {1,2,3,4,3};
        int n = arr.length;
        // System.out.println(MCMrecurssion(arr, 1,n-1));
